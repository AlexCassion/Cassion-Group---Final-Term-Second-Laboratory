...
2. **System Performance Under Increasing Load**
   As the number of generated votes increased, the system continued functioning properly because queued messages were buffered before processing. Under normal conditions, votes appeared in Supabase with minimal delay, but during worker downtime, messages accumulated in the queue until the worker service recovered. Once restored, the worker processed the queued votes in batches, which demonstrated how distributed systems maintain throughput and reliability even when one component temporarily fails.

3. **Challenges Encountered During Implementation and Deployment**
   One of the biggest challenges during implementation was integrating all distributed components and ensuring that they communicated correctly. Since the system was asynchronous, errors were harder to trace because failures could occur between the edge nodes, API requests, queue handling, or database writes. Configuring retries, validating payloads, and testing recovery scenarios required careful debugging to make sure votes were not lost and duplicate processing was minimized.
